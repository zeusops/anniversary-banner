...
def generate_banner(red_points=0, blue_points=0, teams=[]):
    ...
    team_red = Image.open('images/team_red.png', 'r')
    team_blue = Image.open('images/team_blue.png', 'r')
    banner.paste(logo, (50, 40), mask=logo)
    banner.paste(team_red, (50, 240), mask=team_red)
    banner.paste(team_blue, (485, 240), mask=team_blue)

    font_points = ImageFont.truetype('DejaVuSans-Bold.ttf', 80)
    draw = ImageDraw.Draw(banner)
    draw.text((200, 245), "{} - {}".format(red_points, blue_points),
              font=font_points)
...
def banner(request):
    ...
    team_side = request.GET.get('team_side', blufor)
    response =  HttpResponse(content_type="image/png")
    teams = (
        (6, "Terry's 10heads", redfor),
        (4, "Monkey men", blufor),
        (4, "Charlies", redfor),
        (3, "Yonkee donkey", redfor),
        (team_points, team_name, team_side),
    )
    img = generate_banner(red_points, blue_points, teams)
